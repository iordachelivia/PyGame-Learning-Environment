...
#Modify to support 3 actions : up, down, none
...
class HelicopterPlayer(pygame.sprite.Sprite):
    ...
    def update(self, is_climbing, is_falling, dt):
        self.momentum = 0

        if is_climbing:
            self.momentum = (self.climb_speed) * dt
        elif is_falling:
            self.momentum = (self.fall_speed) * dt
...
class Pixelcopter_v2(PyGameWrapper):
    """
    Parameters
    ----------
    width : int
    ...
    def step(self, dt):
        ...
        self.player.update(self.is_climbing, self.is_falling, dt)
...
if __name__ == "__main__":
    ...
    pygame.init()
    game = Pixelcopter_v2(width=256, height=256)
